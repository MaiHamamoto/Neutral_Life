<?php
/*
 * ファイルパス：/Applications/MAMP/htdocs/DT/shopping/lib/Common.class.php
...
class Common
{
    ...
    private function birthCheck()
    {
        ...
        if ($this->dataArr['day'] === '') $this->errArr['day'] = '生年月日の日を選択してください';
    ...
    {
        if ($this->dataArr['address'] === '') $this->errArr['address'] = '住所を入力してください';
    }
    ...
    private function mailCheck()
    {
        ...
        // 重複したメールアドレスを取得するSQL文
        $sql = "select email from member where email = '".$this->dataArr['email']."'";
        // SQL（SELECT文）を実行する
        // $resultは配列 
        $result = $this->db->select($sql);
        // 実行結果が１件以上の場合、エラーを表示
        if (count($result) >= 1){
            $this->errArr['email'] = 'そのメールアドレスはすでに使われています';
        }
    }

    private function passwordCheck()
    {
        if ($this->dataArr['password'] === '' || strlen($this->dataArr['password']) > 8) $this->errArr['password'] = '8ケタ以上のパスワードを入力してください';
    }
